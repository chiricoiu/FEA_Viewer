...
def mesh_surface(liststack, listbiblimesh, listsurfacearea):
    listmesh = []
    rho_t300, e_t300 = 1540, 0.21   # kg/m3, mm
    ...
    for surface in listsurfacearea:
        ...
        nb_t300, nb_t700, rho_mousse, e_mousse = 0, 0, 0, 0
        for ply in stack:
            mat = ply[0]
            if mat == 'T300':
                nb_t300 += 1
...
if __name__ == '__main__':
    ...
    List_TW = tsaiwudf(charger_data_tw())
    print("List_TW : ", time.time() - start, " (s)")
    List_Mesh = meshdf(charger_data_meshsurf())
    print("List_Mesh : ", time.time() - start, " (s)")

    Data = joindata(List_Aire_Masse, List_TW, List_Mesh)
    print("jointure : ", time.time() - start, " (s)")

    datapath = data_dir()
    create_database(datapath, Data)
